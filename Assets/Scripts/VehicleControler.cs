...
public class VehicleControler : MonoBehaviour {
    ...
    void Start() {
        ...
        pickMat.EnableKeyword("_EMISSION");
        ...
        if (number == 0) {
            ...
            Transform carTrails = GameObject.Find("Car").transform;
            trails = carTrails.GetComponentsInChildren<TrailRenderer>();
        }
        ...
        else if (number == 1)
        {
            ...
            trails = Ae.GetComponentsInChildren<TrailRenderer>();
        }
    ...
    void Update() {
        pickMat.SetColor("_EmissionColor", Color.HSVToRGB(Mathf.PingPong(Time.time * 0.4f, 1), 1, 1f) * 2);
        ...
        if(number == 0)
            for (int i = 0; i < objMat.Length; i++) {
                objMat[i].SetColor("_EmissionColor", currentCol * 0.66f);
                if(i == 1)
                    objMat[i].SetColor("_EmissionColor", currentCol * 1.5f);

                if(i == 4 || i == 5)
                    objMat[i].SetColor("_EmissionColor", currentCol * 1.2f);
            }

        if(number == 1)
            for (int i = 0; i < objMat.Length; i++) {
                objMat[i].SetColor("_EmissionColor", currentCol * 0.78f);
                if (i == 1)
                    objMat[i].SetColor("_EmissionColor", currentCol * 1.5f);
    ...
    void FixedUpdate() {
            ...
            if (Physics.Raycast(thrustPos[i], -transform.up, out ray, hovorHeight + 2, mask)) {
                ...
                if (shift) {
                    if (Vector3.Magnitude(entity.velocity) > 10)
                        ...
                        entity.AddTorque(transform.forward * -axis.y * entity.mass * 1400 * Time.deltaTime);
                } else
        ...
        if(!boosting && !GroundBoosting && !oof)
            changeBack();
    }
    ...
    public void changeRed() {
        if (number == 0)
            for (int i = 1; i < mat.Length; i++) {
                mat[i].SetColor("_EmissionColor", Color.Lerp(mat[i].GetColor("_EmissionColor"), Color.red * 1.3f, 0.2f));
                currentCol = Color.Lerp(mat[i].GetColor("_EmissionColor"), Color.red, 0.3f);
            }

        if (number == 1)
            for (int i = 0; i < mat.Length; i++) {
                mat[i].SetColor("_EmissionColor", Color.Lerp(mat[i].GetColor("_EmissionColor"), Color.red * 1.1f, 0.2f));
    ...
    public void OutOfTime() {
        ...
        if (number == 0)
            for (int i = 1; i < mat.Length; i++) {
                mat[i].SetColor("_EmissionColor", Color.Lerp(originColor, Color.red * 1.3f, Mathf.PingPong(Time.time * 2, 1)));
                currentCol = Color.Lerp(originColor, Color.red * 1.3f, Mathf.PingPong(Time.time * 2, 1));
            }

        if (number == 1)
            for (int i = 0; i < mat.Length; i++) {
                mat[i].SetColor("_EmissionColor", Color.Lerp(originColor, Color.red, Mathf.PingPong(Time.time * 2, 1)));
                currentCol = Color.Lerp(originColor, Color.red, Mathf.PingPong(Time.time * 2, 1));
            }
    }

    public void notOutOfTime() {
        oof = false;
    }
    ...
    public void changeBack() {
        if (number == 0)
            for (int i = 1; i < mat.Length; i++) {
                ...
                currentCol = Color.Lerp(mat[i].GetColor("_EmissionColor"), originColor, 0.03f);
            }
